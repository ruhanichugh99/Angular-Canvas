...
export class FabricjsEditorComponent implements AfterViewInit {
  ...
  sharedCanvasOther(email) {
    ...
    this.authService.get_all_employee().subscribe(data=>{
      ...
      data.map(e=>{
        if(e.payload.doc.data()['email'] ==email)
        {
          userExists = true;
          console.log(e.payload.doc.id);
          userId = e.payload.doc.id;
          allSharedCanvas = e.payload.doc.data()['sharedCanvas'];
          var newDt = new Date();
          // console.log(newDt.getDate(),newDt.getMonth())
          var minutes = newDt.getMinutes().toString();
          if(minutes.length == 1)
          {
            minutes = "0"+minutes;
          }
          allSharedCanvas.push({name:'Canvas '+newDt.getDate()+'-'+newDt.getMonth()+'-'+newDt.getUTCFullYear()+'('+newDt.getHours()+':'+minutes+')', value:json})
          // this.authService.updateSharedCanvasOtherUser(e.payload.doc.id,{sharedCanvas:'samarth'});
          return;
        }
    ...
    setTimeout(()=>{
      if(userExists)
      {
        ...
        console.log(allSharedCanvas);
        if(localStorage.getItem('recievedCanvasType')=='true')
        {
          alert("Recieved Canvas cannot be shared");
        }
        else{
          this.authService.updateSharedCanvasOtherUser(userId,{sharedCanvas:allSharedCanvas}); 
          alert('Canvas can be shared')
        }
        
      }
      else{
        console.log("User does not exists");
        alert("User does not exists in canvas database....."+email+' must login first');
      }
    },1000)
  }

  getAllSharedCanvas() : any
  {
    let userExists = false;
    let userId;
    let allSharedCanvas = [];
  ...
  loadCanvasFromJSON() {
    ...
    localStorage.setItem('recievedCanvasType','false');
    let CANVAS;
    this.authService.get_all_employee().subscribe(data=>{
      data.map(e=>{
        if(e.payload.doc.id == localStorage.getItem('userId'))
        {
          // console.log('MY User')
          // console.log(e.payload.doc.data()['myCanvas'])
          CANVAS = e.payload.doc.data()['myCanvas'];
          console.log('CANVAS');
          console.log(CANVAS);
  ...
  loadRecievedCanvasFromJSON(json) {
    // const CANVAS = localStorage.getItem('Kanvas');
    ...
    console.log(CANVAS);
    localStorage.setItem('recievedCanvasType','true');
        // and load everything from the same json
    this.canvas.loadFromJSON(CANVAS, () => {
